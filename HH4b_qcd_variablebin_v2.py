...
iPeriod =4
weight=["(trigger_function(766))","(trigger_function_lower(721))","(trigger_function_upper(814))"]
weightname=["","low","up"]
# FORMAT IS:
...
for w in weight:
	...
	QCD = DIST("Data", "QCD_HT_v6p4.root", "myTree", w)
	...
	C2.Print(weightname[index]+"fi_bbtag%s.pdf"%cut[0])
	...
	FILE = TFile("Hbb_output"+weightname[index]+".root", "RECREATE")
	FILE.cd()
	...
	FILE.Write()
	FILE.Save()
	C3.Print(weightname[index]+"_bkg_bbtag%s.pdf"%cut[0])
	...
	C4.cd()
	C4.Update()
	C4.Print(weightname[index]+"_pull.pdf")
