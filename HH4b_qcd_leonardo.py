...
for b in range(len(binBoundaries)-1):
	preselection=presel+"&&dijetmass>"+str(binBoundaries[b])+"&&dijetmass<"+str(binBoundaries[b+1])
	print preselection


	HbbTest = Alphabetizer("QCDalphaTest"+str(b), DistsWeWantToEstiamte, [])
...
# pick the two variables to do the estiamte it (in this case, Soft Drop Mass (from 70 to 350 in 48 bins) and tau32 (from 0 to 1))
	...
	HbbTest.SetRegions(var_array, preselection) # make the 2D plot
...
#	F = LinearFit([0.2,-0.2], -75, 75, "linFit1", "EMRNS")
	...
	C2.Print("bin_"+str(binBoundaries[b])+"_"+str(binBoundaries[b+1])+"fi_bbtag%s.pdf"%cut[0])
	listofhbbtests.append(copy.copy(HbbTest))
...
for b in range(len(binBoundaries)-1):
	...
	tag = "(dijetmass>800&(jet2pmass<130&jet2pmass>90)&(jet1pmass<130&jet1pmass>100)&jet1tau21<0.6&jet2tau21<0.6&(jet1bbtag>0.4&jet2bbtag>0.4))"+"&&dijetmass>"+str(binBoundaries[b])+"&&dijetmass<"+str(binBoundaries[b+1])
	antitag = "(dijetmass>800&(jet2pmass<130&jet2pmass>90)&(jet1pmass<130&jet1pmass>100)&(jet1tau21<0.6&jet2tau21<0.6)&(jet1bbtag>0.4&jet2bbtag<0.4))"+"&&dijetmass>"+str(binBoundaries[b])+"&&dijetmass<"+str(binBoundaries[b+1])
	print str(binBoundaries[b])+"<dijetmass<"+str(binBoundaries[b+1])

	
	Test_alpha=listofhbbtests[b]
	print b, listofhbbtests[b].name
	Test_alpha.MakeEstVariable(variable, binBoundaries, antitag, tag)
	...
	for i in Test_alpha.hists_MSR:
		V.Add(i,1.)
	# the estimate is the sum of the histograms in self.hists_EST and self.hist_MSR_SUB
	for i in Test_alpha.hists_EST:
		N.Add(i,1.)
	# We can do the same thing for the Up and Down shapes:	
	for i in Test_alpha.hists_EST_UP:
		NU.Add(i,1.)
	for i in Test_alpha.hists_EST_DN:
		ND.Add(i,1.)
	for i in Test_alpha.hists_ATAG:
		A.Add(i,1.)
